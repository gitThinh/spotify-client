...
const Playing = ({ handleNextSong, userid }) => {
  ...
  useEffect(() => {
    ...
    newSourceElement.src = playingSong
      ? `${urlApiSong + playingSong.file_name}`
      : "";
  ...
  // next and prev btn
  const handleNextBtn = () => {
    israndom ? handleNextSong(1) : handleNextSong(0);
  };
  const handlePrevBtn = () => {
    israndom ? handleNextSong(1) : dispatch(actions.prevSong());
  };
  ...
  useEffect(() => {
    function updateProgressFromMousePosition(event) {
      ...
      const clickX =
        event.clientX - progressContainer.current.getBoundingClientRect().left;
      ...
      if (files.length > 0) {
        ...
        if (file.type.includes("audio")) {
          ...
          reader.onload = (event) => {
            ...
            totalDurationDisplay.current.innerHTML = formatTime(
    ...
    audioRef.current.addEventListener("timeupdate", () => {
      if (audioRef.current.ended) {
        ...
        currentTimeDisplay.current.innerHTML = formatTime(
  ...
  useEffect(() => {
    ...
    function updateVolumeFromMousePosition(event) {
      ...
      if (volumeContainerWidth > volumeContainerHeight) {
        volumeClickX =
          event.clientX - volumeContainer.current.getBoundingClientRect().left;
        volumeControl = volumeClickX / volumeContainerWidth;
      } else {
        volumeClickY =
          event.clientY - volumeContainer.current.getBoundingClientRect().top;
    ...
    volumeContainer.current.addEventListener("drop", (e) => {
      ...
      if (
        volumeContainer.current.clientWidth >
        volumeContainer.current.clientHeight
      ) {
        offsetX =
          e.clientX - volumeContainer.current.getBoundingClientRect().left;
        setVolumes(offsetX / volumeContainer.current.offsetWidth);
      } else {
        offsetY =
          e.clientY - volumeContainer.current.getBoundingClientRect().top;
    ...
    <div className="playing_block noone_coppy">
      <div className="info_music">
        {playingSong && (
          <div className="box_info">
            <img
              src={(playingSong && `${urlApiImg + playingSong.coverArt}`) || ""}
              alt="thumbnail"
            />
      ...
      <div className="playing_control">
        <div className="btn_playing_control">
          <div
            className={israndom ? "btn btn_random active" : "btn btn_random"}
          ...
          <div
            ...
            onClick={audioRef.current ? handlePrevBtn : () => {}}
          >
          ...
          {isloading ? (
            ...
            <div
              className="btn btn_toggle_play"
              onClick={audioRef.current ? handPlayPause : () => {}}
            >
              {isplaying ? (
          ...
          <div
            ...
            onClick={audioRef.current ? handleNextBtn : () => {}}
          >
            <FaForwardStep className="btn_next_icon" />
          </div>
          <div
            className={"btn btn_repeat".concat(" ", isrepeat ? "active" : "")}
            onClick={() => setIsreapet(!isrepeat)}
          >
            <FaArrowRotateRight className="btn_repeat_icon" />
          </div>
        </div>
        <div className="time_line">
          <p className="start_time" ref={currentTimeDisplay}>
            --:--
          </p>
